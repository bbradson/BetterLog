...
public static class FishTranspiler
{
	public struct Container
	{
		...
		public CodeInstruction WithBlocks(IEnumerable<ExceptionBlock> blocks) => ToInstruction().WithBlocks(blocks);
		public CodeInstruction WithLabelsAndBlocks(CodeInstruction instruction) => WithLabels(instruction.labels).WithBlocks(instruction.blocks);
		...
		public override int GetHashCode() => Operand is null ? OpCode.GetHashCode() : unchecked((((1009 * 9176) + OpCode.GetHashCode()) * 9176) + Operand.GetHashCode());
	...
	public static Container New(Type type, Type[]? parameters = null, bool searchForStatic = false) => New(AccessTools.Constructor(type, parameters, searchForStatic));
	public static Container New(ConstructorInfo constructor)
		=> constructor != null ? new() { OpCode = OpCodes.Newobj, Operand = constructor }
	...
	public static IEnumerable<CodeInstruction> MethodReplacer<T, V>(this IEnumerable<CodeInstruction> instructions, T from, V to, bool throwOnFailure = true) where T : Delegate where V : Delegate
	{
		if (from is null)
		...
		if (throwOnFailure && !success)
			throw new($"FishTranspiler.MethodReplacer couldn't find method {to.Method.FullDescription()} for {Assembly.GetExecutingAssembly().GetName().Name}, Version {Assembly.GetExecutingAssembly().GetName().Version}");
	}

	public static IEnumerable<CodeInstruction> InsertBefore(this IEnumerable<CodeInstruction> instructions, Predicate<CodeInstruction> predicate, CodeInstruction instructionToInsert, bool throwOnFailure = true)
		=> instructions.InsertBefore(predicate, new[] { instructionToInsert }, throwOnFailure);
	public static IEnumerable<CodeInstruction> InsertBefore(this IEnumerable<CodeInstruction> instructions, Predicate<CodeInstruction> predicate, CodeInstruction[] instructionsToInsert, bool throwOnFailure = true)
	{
		if (predicate is null)
			throw new ArgumentException("Unexpected null argument", nameof(predicate));
		if (instructionsToInsert is null || !instructionsToInsert.Any())
			throw new ArgumentException("Unexpected null argument", nameof(instructionsToInsert));
		var success = false;
		...
		if (throwOnFailure && !success)
			throw new($"FishTranspiler.InsertBefore couldn't find target instruction for {predicate.Method.FullDescription()} of {Assembly.GetExecutingAssembly().GetName().Name}, Version {Assembly.GetExecutingAssembly().GetName().Version}");
	}

	public static IEnumerable<CodeInstruction> InsertAfter(this IEnumerable<CodeInstruction> instructions, Predicate<CodeInstruction> predicate, CodeInstruction instructionToInsert, bool throwOnFailure = true)
		=> instructions.InsertAfter(predicate, new[] { instructionToInsert }, throwOnFailure);
	public static IEnumerable<CodeInstruction> InsertAfter(this IEnumerable<CodeInstruction> instructions, Predicate<CodeInstruction> predicate, CodeInstruction[] instructionsToInsert, bool throwOnFailure = true)
	{
		if (predicate is null)
			throw new ArgumentException("Unexpected null argument", nameof(predicate));
		if (instructionsToInsert is null || !instructionsToInsert.Any())
		...
		if (throwOnFailure && !success)
			throw new($"FishTranspiler.InsertAfter couldn't find target instruction for {predicate.Method.FullDescription()} of {Assembly.GetExecutingAssembly().GetName().Name}, Version {Assembly.GetExecutingAssembly().GetName().Version}");
	}

	public static IEnumerable<CodeInstruction> Replace(this IEnumerable<CodeInstruction> instructions, Predicate<CodeInstruction> predicate, Func<CodeInstruction, CodeInstruction> replacement, bool throwOnFailure = true)
		=> instructions.Replace(predicate, c => new[] { replacement(c) }, throwOnFailure);
	public static IEnumerable<CodeInstruction> Replace(this IEnumerable<CodeInstruction> instructions, Predicate<CodeInstruction> predicate, Func<CodeInstruction, CodeInstruction[]> replacement, bool throwOnFailure = true)
	{
		if (predicate is null)
			throw new ArgumentException("Unexpected null argument", nameof(predicate));
		if (replacement is null)
			throw new ArgumentException("Unexpected null argument", nameof(replacement));
		var success = false;
		...
		if (throwOnFailure && !success)
			throw new($"FishTranspiler.Replace couldn't find target instruction for {predicate.Method.FullDescription()} of {Assembly.GetExecutingAssembly().GetName().Name}, Version {Assembly.GetExecutingAssembly().GetName().Version}");
	}

	public static IEnumerable<CodeInstruction> ReplaceAt(this IEnumerable<CodeInstruction> instructions, Func<List<CodeInstruction>, int, bool> position, Func<CodeInstruction, CodeInstruction> replacement, bool throwOnFailure = true)
		=> instructions.ReplaceAt(position, c => new[] { replacement(c) }, throwOnFailure);
	public static IEnumerable<CodeInstruction> ReplaceAt(this IEnumerable<CodeInstruction> instructions, Func<List<CodeInstruction>, int, bool> position, Func<CodeInstruction, CodeInstruction[]> replacement, bool throwOnFailure = true)
	{
		if (position is null)
			throw new ArgumentException("Unexpected null argument", nameof(position));
		if (replacement is null)
			throw new ArgumentException("Unexpected null argument", nameof(replacement));
		var success = false;
		...
		for (var i = 0; i < codes.Count; i++)
		{
			if (position(codes, i))
			{
				var code = codes[i];
				codes.RemoveAt(i);
				codes.InsertRange(i, replacement(code));
				success = true;
			}
		}

		return throwOnFailure && !success
			? throw new($"FishTranspiler.ModifyAt couldn't find target instruction for {position.Method.FullDescription()} of {Assembly.GetExecutingAssembly().GetName().Name}, Version {Assembly.GetExecutingAssembly().GetName().Version}")
			: codes;
	}

	public static CodeInstruction With(this CodeInstruction instruction, OpCode? opcode = null, object? operand = null, IEnumerable<Label>? labels = null, IEnumerable<ExceptionBlock>? blocks = null)
	{
		if (opcode != null)
